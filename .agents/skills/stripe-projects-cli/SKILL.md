...
This repository is initialized for the Stripe project "okta_stripe_hack".
...
|------|---------|------------|
| `BROWSER_AUTH_REQUIRED` | No Stripe session and browser auth needed | Run `stripe login --non-interactive` to print JSON with `browser_url`, `verification_code`, and `next_step`; present `browser_url` and `verification_code` to the user, then run the emitted `next_step` command to complete login before retrying. |
| `BROWSER_AUTH_TIMEOUT` | Browser auth did not complete in time | Ask the user to finish the browser flow, then retry |
| `ACCOUNT_NOT_ELIGIBLE` | Account not onboarded for Projects | Visit https://projects.dev to enable Projects on this account, or run `stripe projects switch-account` to use a different account. |
